...
"""
function taxonomicdistance!(
    D::Matrix{Float64},
    tax::Vector{NCBITaxon};
    d::Dict{Symbol, Float64} = Dict(
        :species => 0.0,
        :genus => 1.0,
        :family => 2.0,
        :subclass => 3.0,
    ...
    if size(D, 1) != length(tax)
        throw(
            DimensionMismatch(
                "The matrix `D` ($(size(D,1))) must have the same sizes as the length of `tax` ($(length(tax))).",
            ),
        )
    end
